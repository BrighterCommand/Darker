...
- [x] **TEST + IMPLEMENT: FR14 — `QueryLoggingJsonOptions.Options` locks after first query and rejects subsequent mutation (AC3)** — test approved (GREEN-on-arrival contract pin; no production code — FR14 is the System.Text.Json lock-after-first-use invariant plus the Step-2 "class-init must not lock" guarantee). Added `Logging/When_QueryLoggingJsonOptions_Options_mutated_after_first_query_executes_should_throw_InvalidOperationException.cs` in `[Collection("QueryLoggingJsonOptionsOrdering")]`: single sequential `[Fact]` walking step 1 `MaxDepth = 32` succeeds → step 2 `builder.AddJsonQueryLogging(o => o.WriteIndented = true)` succeeds (the canonical `QueryProcessorBuilder` fluent path, so `AddJsonQueryLogging` both invokes the configure callback against `QueryLoggingJsonOptions.Options` and registers the decorators) → step 3 `Execute(new OrderingTestQuery())` serialises + locks; captured `{Query}` (line-endings normalised per finding #10) equals the indented `"{\n  \"Marker\": \"x\"\n}"` (proves the `WriteIndented` callback took effect) → step 4 `AddJsonQueryLogging(o => o.WriteIndented = false)` throws `InvalidOperationException`, surfaced unmodified. **Deviation from AC3's "run last / shared default" framing (decision #5 / C5)**: the lock is isolated on a **throwaway** options instance set at test start and discarded in `finally`, so it never locks the shared default — the irreversible lock cannot leak across tests, which also makes the fragile collection-ordering hazard (finding #14) moot. Core.Tests 76 pass on net8.0 + net9.0 (was 75, +1), no regression, no leak.
...
- [x] **STRUCTURAL: Verify sample app still demonstrates query logging end-to-end (AC5)** — verification-only, no code changes (so `/tidy-first` had nothing to refactor). `dotnet build samples/SampleMinimalApi/SampleMinimalApi.csproj -c Release` → 0 errors (24 warnings = the documented pre-existing pipeline/registry IL sites, out of scope). Ran `dotnet run … -f net9.0` (multi-TFM needs `--framework`; launchSettings bound it to http://localhost:5258). **`GET /people`** → `info: …QueryLoggingDecoratorAsync` `"Executing async query GetPeopleQuery: {}"` then `"Async execution of query GetPeopleQuery completed in 98.2527ms"` — `{Query}` body is exactly `{}` for the parameterless query. **`GET /people/999`** (unknown id) → `"Executing async query GetPersonNameQuery: {\"PersonId\":999}"` (STJ-serialised), `FallbackPolicyDecoratorAsync` fired, completion line is the with-fallback variant `"Async execution of query GetPersonNameQuery completed in 157.6134ms (with fallback)"`. **AC1 cross-check**: `dotnet list … package --include-transitive` on net8.0 + net9.0 shows `System.Text.Json 10.0.8` and **no `Newtonsoft.Json`**.
...
- [x] **DOC: Draft V5 migration entry covering serialiser swap (DoD)** — authored `specs/006-json_serializer/release-notes-draft.md`. Covers every DoD item with a dedicated heading/bullet: (1) callback type `Action<JsonSerializerSettings>`→`Action<JsonSerializerOptions>`; (2) decorator ctor change (no serialiser param); (3) DI singleton removal; (4) Newtonsoft removed from `Paramore.Darker`; recommended migration snippet (preserves `IgnoreCycles`, from requirements DoD 279–281) + direct-assignment snippet (warns it drops `IgnoreCycles`, from DoD 285–289); swap-serialiser-entirely → custom decorator (ADR Alt 1 dismissal); AOT note (IL2026/IL3050 FR13 local-suppression policy + `TypeInfoResolver` opt-in, logging path AOT-clean verified by the native-AOT console harness); known AOT limitation = the ~28 pre-existing pipeline/registry trim warnings outside `Logging/` (cited file:line, **not** suppressed); parallel-`WebApplicationFactory` C6 `DisableParallelization` recommendation; builder-surface `NotSupportedException` limitation. Folded in this session's two consumer-guidance discoveries: `[DynamicDependency(PublicMethods, typeof(handler))]` rooting for full-trim AOT, and the `QueryLoggingJsonOptions.Options` process-global lock-on-first-serialize ("configure once at startup"). Explicitly states the xunit v2→v3 upgrade is **not** a consumer break.
...
- [x] **STRUCTURAL: Cross-cutting validation against all acceptance criteria** — full gate run 2026-06-04, all checks pass. **Build** `dotnet build Darker.Filter.slnf -c Release` → 0 errors (AC1, net8.0+net9.0). **Test** `dotnet test Darker.Filter.slnf -c Release --no-build` → Core **76**, Extensions **8**, all green on both TFMs, 0 failed/skipped (AC2; includes the rewritten extension test, the rewritten core decorator test, and the FR14 ordering test — AC3). **AOT** `dotnet publish … -f net8.0 -r osx-arm64` and `-f net9.0 -r osx-arm64` → publish exit 0, **zero IL2xxx/IL3xxx under `src/Paramore.Darker/Logging/`** outside the FR13 allow-list; both published binaries run to **exit 0** (`[Case1] PASS` + `[Case2] PASS`) — AC4. **Newtonsoft absence**: `dotnet list src/Paramore.Darker/… package --include-transitive` → NONE on net8.0/net9.0/netstandard2.0; sample `--include-transitive` → NONE on net8.0/net9.0 (re-confirmed Step 11 this session) — AC1. **`Directory.Packages.props`**: `System.Text.Json 10.0.8`, `xunit.v3 3.2.2`, **no** bare `xunit` 2.x, **no** `Newtonsoft.Json` — AC1. **xunit.v3 csprojs**: `Core.Tests` + `Extensions.Tests` reference `xunit.v3`; no csproj references bare `xunit`; benchmarks csproj has no xunit (unchanged). *Note: the tasks-line "four FR12 csprojs" predates the locked Step 9 design change — the AOT project was converted to a native-AOT console harness (no xunit) and `Paramore.Test.Helpers` was deleted, so only 2 test projects now carry xunit.v3. The substantive AC1 invariant (zero xunit 2.x, xunit.v3 pinned, benchmarks unchanged) holds.* **Sample smoke**: run this session in Step 11 against these same Release binaries — `GET /people` → `{}`, `GET /people/999` → with-fallback completion line — AC5. **Release-notes draft**: `specs/006-json_serializer/release-notes-draft.md` exists with a section/bullet per Step-12 DoD item, committed in `d11a0ed`.
