...
- [x] **Async caching decorator + AddCaching: cache hit skips the handler, miss runs it once and populates (minimal vertical slice)**
...
- [x] **Sync caching decorator + attribute: fast-path returns synchronously when the ValueTask is already completed**
...
- [x] **End-to-end metrics chain: a cache miss then hit produce hit/miss counters, and the toggle disables them**
