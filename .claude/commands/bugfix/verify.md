...
2. Run the broader suite to check for regressions:
   `dotnet test Darker.Filter.slnf -c Release`
   (or the touched test project on its own for a faster loop).
...
scope (including any additional defects / sync/async parity twin), and `Fixes #{issue-number}`.
