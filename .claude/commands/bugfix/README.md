...
**Worked example (illustrative).** An issue reports that `[FallbackPolicy]` never catches
exceptions thrown by an async handler, and arrives with a plausible suggested fix: wrap the
handler call in a `try/catch` inside the async decorator. Before writing any code, Confirm traces
the pipeline and proves the *real* cause: handlers invoked by reflection surface their exception
wrapped in a `TargetInvocationException`, which Darker only unwraps via
`ExceptionDispatchInfo.Capture().Throw()` *after* the policy's exception predicate has already
failed to match (see `QueryProcessor.cs:50-54` and `PipelineBuilder.cs:57-61`). The trace also
surfaces a *second* defect the suggested fix misses entirely: the same gap exists on the
**synchronous `Execute` path**, so an async-only fix would leave the sync pipeline broken.
Confirming the theory **changed the scope of the fix.** That is exactly what the Confirm gate
exists to catch. *(This scenario is illustrative — it shows the kind of diagnosis Confirm forces,
not a specific closed issue.)*
...
- an executable **red reproduction** (preferred — most Darker bugs reproduce cheaply because the
  query pipeline is pure in-memory logic with no live infrastructure), **or**
- a documented **code-trace** that proves the cause (the accepted path when a repro would be
  disproportionately expensive to stand up).
...
└── 0001-fallback-async-exceptions/
