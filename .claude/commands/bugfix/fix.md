...
4. **Sync/async pipeline parity**: if the cause touches one of Darker's parallel pipelines
   (`Execute` / sync or `ExecuteAsync` / async) and the Scope Notes flagged a parity twin, apply
   the equivalent fix to the other path so the two pipelines stay in step.
