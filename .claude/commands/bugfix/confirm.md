...
until this passes. Confirming the theory frequently changes the *scope* of the fix (e.g. proving
that a `[FallbackPolicy]` failure is caused by `TargetInvocationException` unwrapping order can
surface a *second* defect on the synchronous `Execute` path that the suggested async-only fix
never touched).
...
- **Evidence standard (red repro preferred, code-trace allowed):**
  - If the bug can be reproduced cheaply (the common case in Darker — the query pipeline is pure
    in-memory logic with no live infrastructure), describe the **red repro**: the exact assertion
    that fails today and why.
  - A documented **code-trace** that proves the cause is also sufficient — the accepted path when
    an executable reproduction would be disproportionately expensive to stand up. The durable
    executable test is written later by `/bugfix:test` — Confirm does not write source.
...
- **Hunt for scope changes**: does the proven cause imply *additional* defects or related call
  sites the suggested fix misses? In particular, check **sync/async pipeline parity** — Darker
  runs parallel `Execute` (sync) and `ExecuteAsync` (async) pipelines, so a cause in one path
  usually has a twin in the other. List anything found — these change the fix's scope.
...
(tick whichever applies; a trace alone is valid when a repro is disproportionately expensive)

## Suggested-Fix Assessment
CONFIRMED / PARTIAL / WRONG — and why.

## Scope Notes
Additional defects, related sites, or sync/async pipeline parity gaps the fix must also cover (or
...
- Any Scope Notes (especially additional defects / sync/async parity gaps — these may widen the fix)
