...
via `Bash`), while still allowing Read/Glob/Grep to verify test/impl paths. The sub-agent reads
the requirements and ADRs and RETURNS the ralph task list as text. The main agent runs the
...
1. **One thing per task**: Single component, decorator, behavior, or migration. Never combine.
