...
- Test behavior, not implementation — public exports only; prefer real/Simple/InMemory* test
  doubles over mocks (Real > Simple > InMemory > Mock).
...
- Run broader tests (`dotnet test Darker.Filter.slnf -c Release` or the relevant project[s])
  to catch regressions. If a regression appears, fix it before finishing.
...
counters in the wake-up prompt) — this is the in-session equivalent of an overnight runner.
