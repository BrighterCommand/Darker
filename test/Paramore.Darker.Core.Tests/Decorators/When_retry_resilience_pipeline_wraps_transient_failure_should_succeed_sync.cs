...
{
    public class When_retry_resilience_pipeline_wraps_transient_failure_should_succeed_sync
    {
        [Fact]
        public void Execute_through_zero_delay_retry_pipeline_should_retry_to_success()
        {
            ...
            // Act
            var result = decorator.Execute(new SyncTestQuery(id),
                q => handler.Execute(q),
                q => null);
