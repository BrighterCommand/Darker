...
2. **Rewrite `QueryLoggingDecorator<TQuery, TResult>` and `QueryLoggingDecoratorAsync<TQuery, TResult>`** to call `JsonSerializer.Serialize(query, QueryLoggingJsonOptions.Options)` at the `Serialize<T>` private method. The decorator constructors no longer take a serialiser parameter. The `ConfigurationException("No serializer settings are configured…")` throw site is removed — `Options` is never `null` at the call site (class-init guarantees it; setter null-guard preserves it).
